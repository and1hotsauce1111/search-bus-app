...
export const queryString = (
  ...
  if (query.filter && query.filter.type === 'bus/route') {
    if (requestUrl === '') {
      requestUrl =
        requestUrl +
        '?' +
        filterMultipleDataString('RouteUID', query.filter.routeUID);
    } else {
      requestUrl =
        requestUrl +
        '&' +
  ...
  if (query.filter && query.filter.type === 'bus/type') {
    if (requestUrl === '') {
      requestUrl =
        requestUrl +
        '?' +
        filterMultipleDataString('PlateNumb', query.filter.plateNumb);
    } else {
      requestUrl =
        requestUrl +
        '&' +
  ...
  if (query.filter && query.filter.type === 'bus/stop') {
    if (requestUrl === '') {
      requestUrl =
        requestUrl + '?' + filterBusByStopString(query.filter.keyword);
    } else {
      requestUrl =
        requestUrl + '&' + filterBusByStopString(query.filter.keyword);
    }
  ...
  if (
    query.filter &&
    (query.filter.type === 'stop' || query.filter.type === 'shape')
  ) {
    if (requestUrl === '') {
  ...
  if (query.filter && query.filter.type === 'bike/stop') {
    if (requestUrl === '') {
      requestUrl = requestUrl + '?' + filterBikeStation(query.filter.keyword);
    } else {
      requestUrl = requestUrl + '&' + filterBikeStation(query.filter.keyword);
    }
  }
  if(query.filter && query.filter.type === 'bike/availbility') {
    if (requestUrl === '') {
      requestUrl =
        requestUrl +
        '?' +
        filterMultipleDataString('StationUID', query.filter.StationUID);
    } else {
      requestUrl =
        requestUrl +
        '&' +
